...
### 7. GIS Map View
แผนที่ต้นไม้รายต้นในแต่ละโครงการ (tab "แผนที่" ในหน้ารายละเอียดโครงการ) ปักหมุดตามพิกัดจริง สีตาม DBH class (4 ช่วง), filter ชนิดพืช/DBH/สถานะ, สลับ base layer (ภาพถ่ายดาวเทียม ESRI / ถนน OSM / ภูมิประเทศ OpenTopoMap), scroll-wheel zoom, marker clustering กันหน่วงเมื่อหมุดเยอะ — ใช้ตรวจสอบย้อนกลับถึงแปลงจริงสำหรับ VVB spot-check

มี layer เสริมเปิด/ปิดได้ "ตำแหน่งเซนเซอร์แนะนำ" ที่ซ้อนผลลัพธ์จาก `/api/sensor-plan` (คำนวณจากพื้นที่จริง+ชนิดพืชจริงของโครงการ) เป็นกริดจุดตัวอย่างรอบจุดศูนย์กลาง — เป็นภาพประกอบช่วยเล่าฟีเจอร์เซนเซอร์เท่านั้น ไม่ใช่ตำแหน่งจริงทั่วทั้งแปลง (ไม่มีข้อมูลขอบเขตแปลงจริง จึง cap ไว้ที่ 150 จุดเพื่อความชัดเจนของภาพ แม้ตัวเลข totalSensors ที่คำนวณได้จริงอาจมีหลักพัน)
